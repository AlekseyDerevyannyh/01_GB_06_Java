...
public class ViewUser {
    ...
    public void checkBirthDate(String birthDateString) throws DateFormatException {
        ...
        if (birthDateData.length != NUMBER_OF_DATE_DATA_FIELDS) {
            throw new DateFormatException("Ошибка! Неверный формат даты (dd.mm.yyyy): количество полей, разделённых точкой (.), не равно трём!");
        }
    ...
    public void checkPhone(String phone) throws DateFormatException {
        if (!isNumeric(phone)) {
            throw new DateFormatException("Ошибка! Неверный формат телефонного номера: номер телефона должен состоять только из цифр");
        }
    }

    public void checkSex(String sex) throws DateFormatException {
        if (!sex.equalsIgnoreCase("f") && !sex.equalsIgnoreCase("m")) {
            throw new DateFormatException("Ошибка! Неверный формат пола: должен быть 'f' или 'm'");
        }
    }

    public void checkDay(String day) throws DateFormatException {
        if (day.length() != DAY_FIELD_SIZE) {
            throw new DateFormatException("Ошибка! Неверный формат даты (dd.mm.yyyy): длина поля день (dd) не равна двум!");
        }
        if (!isNumeric(day)) {
            throw new DateFormatException("Ошибка! Неверный формат даты (dd.mm.yyyy): поле день (dd) должно быть числом!");
        }
        if (Integer.parseInt(day) < 1 || Integer.parseInt(day) > 31) {
            throw new DateFormatException("Ошибка! Неверный формат даты (dd.mm.yyyy): поле день (dd) должно быть от 01 до 31");
        }
    }
    public void checkMonth(String month) throws DateFormatException {
        if (month.length() != MONTH_FIELD_SIZE) {
            throw new DateFormatException("Ошибка! Неверный формат даты (dd.mm.yyyy): длина поля месяц (mm) не равна двум!");
        }
        if (!isNumeric(month)) {
            throw new DateFormatException("Ошибка! Неверный формат даты (dd.mm.yyyy): поле месяц (mm) должно быть числом!");
        }
        if (Integer.parseInt(month) < 1 || Integer.parseInt(month) > 12) {
            throw new DateFormatException("Ошибка! Неверный формат даты (dd.mm.yyyy): поле месяц (mm) должно быть от 01 до 12");
        }
    }

    public void checkYear(String year) throws DateFormatException {
        if (year.length() != YEAR_FIELD_SIZE) {
            throw new DateFormatException("Ошибка! Неверный формат даты (dd.mm.yyyy): длина поля год (yyyy) не равна двум!");
        }
        if (!isNumeric(year)) {
            throw new DateFormatException("Ошибка! Неверный формат даты (dd.mm.yyyy): поле год (yyyy) должно быть числом!");
        }
        LocalDate localDate = LocalDate.now();
        if (Integer.parseInt(year) > localDate.getYear()) {
            throw new DateFormatException("Ошибка! Неверный формат даты (dd.mm.yyyy): поле год (yyyy) должно быть от 0000 до " + localDate.getYear());
        }
    }
